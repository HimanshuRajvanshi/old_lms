...
<div id="content">
    ...
    <div class="right_content" style="width:780px">  <!-- Start of right content-->    
...
{
?>
<input name="el" id="el_<?php echo $row['balid']; ?>" type="text" class="el_<?php echo $row['balid']; ?>" value="<?php echo $row['el'];  ?>" style="width:50px" disabled="disabled" />
<?php
}
else {
?>
<input name="el" id="el_<?php echo $row['balid']; ?>" type="text" class="el_<?php echo $row['balid']; ?>" value="<?php echo $row['el'];  ?>" style="width:50px" />
<?php
...
<td class="balance_<?php echo $row['balid']; ?>"><?php echo $row['balance'];  ?> </td>

<td class="availed_<?php echo $row['balid']; ?>"><?php echo $row['availed'];  ?> </td>


<td><input type="text" style="width:50px" value="<?php echo $row['total'];  ?>" class="total_<?php echo $row['balid']; ?>" id="total_<?php echo $row['balid']; ?>" /> </td>
...
<input id="empid_<?php echo $row['balid']; ?>" name="empid" type="hidden" value="<?php echo $row['empid']; ?>" >
</tr>
<script type="text/javascript" >
...
$("#total_<?php echo $row['balid']; ?>").blur(function(event)
{
event.preventDefault(event);
var cl = $("#cl_<?php echo $row['balid']; ?>").val();
var el = $("#el_<?php echo $row['balid']; ?>").val();
var empid = $("#empid_<?php echo $row['balid']; ?>").val();
var total = $("#total_<?php echo $row['balid']; ?>").val(); 


	$.post("updateentitle.php",{cl:cl, el:el, empid:empid, total:total},  function(data){
	
	 var result =$.parseJSON(data);
	 
	 

  $(".balance_<?php echo $row['balid']; ?>").html(result.balance).css("font-size", "25px").effect("pulsate", { times:4 }, 1500);
      $(".availed_<?php echo $row['balid']; ?>").html(result.availed).css("font-size", "25px").effect("pulsate", { times:4 }, 1500);  });
$(".total_<?php echo $row['balid']; ?>").html(result.total);
});

$("#cl_<?php echo $row['balid']; ?>").blur(function(event)
{
event.preventDefault(event);
var cl = $("#cl_<?php echo $row['balid']; ?>").val();
var el = $("#el_<?php echo $row['balid']; ?>").val();
...
$("#el_<?php echo $row['balid']; ?>").blur(function(event)
{
event.preventDefault(event);
var cl = $("#cl_<?php echo $row['balid']; ?>").val();
var el = $("#el_<?php echo $row['balid']; ?>").val();
