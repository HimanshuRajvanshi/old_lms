...
$sql = "SELECT * FROM  `employee` where userstatus = 'active' order by fname ";
$res = mysql_query($sql)or die(mysql_error());
while($row = mysql_fetch_array($res))
{

	if($_POST['emp'] == $row['empid'])
	{
	echo '<option value='.$row['empid'].' selected=selected>'.$row['fname'].' '.$row['lname'].'</option>';
	}
	else
	{
...
<input type="text" id="startdate" name="startdate" value="<?php echo $_POST['startdate']; ?>"> <input type="text" id="enddate" name="enddate" value="<?php echo $_POST['enddate']; ?>">
...
$query = "SELECT   SUM(`daycount`) as total  FROM `leave` WHERE empid = '".$_POST['emp']."' and STR_TO_DATE(`fromdate`, '%m/%d/%Y') BETWEEN str_to_date('".$_POST['startdate']."','%m/%d/%Y') AND str_to_date('".$_POST['enddate']."','%m/%d/%Y') and status = 1";
$daycount = mysql_query($query)or die(mysql_error());
$sumobj = mysql_fetch_object($daycount);

$sql = "SELECT leavetype, reqname, daycount, fromdate, todate, reason FROM `leave` WHERE empid = '".$_POST['emp']."' and STR_TO_DATE(`fromdate`, '%m/%d/%Y') BETWEEN str_to_date('".$_POST['startdate']."','%m/%d/%Y') AND str_to_date('".$_POST['enddate']."','%m/%d/%Y') and status = 1";
...
echo '<tr><td></td><td></td><td></td><td class="rowhead">Total Leaves</td><td colspan="2" class="rowhead">'.$sumobj->total.'</td></tr>';
}
