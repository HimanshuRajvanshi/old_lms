...
$count = $_GET['count'];
   ...
   if($mess == 'exist')
  {
   ...
   if($mess == 'userexist')
  {
   echo $count.' employee(s) need a new supervisor email before making inactive.';
  }
  ?></label>
  <?php
    if($mess == 'success')
...
$sql ="Select * from employee, department, usergroup where empid = '".$id."'AND employee.type=usergroup.groupid AND employee.department=department.depid";
  ...
  ?>
        ...
        <input id="doj" type="text" class="required" name="doj" value="<?php   
           ...
           echo $mysqldate = date("Y-m-d", $datetime);?>" style="width:100px; display:inline" />
   
     <label for="empcode">
            Employee Code *</label>
        <input id="empcode" type="text" class="required" name="empcode" value="<?php echo $row->empcode; ?>" style="width:100px; display:inline" />
         <label for="supemail">
  ...
  ?> 
          ...
          <label for="status">Select Employee Type *</label>
    ...
    <option  value="prob" <?php echo ($row->status == 'prob')?"selected='selected'":"" ?>>Probation</option>
		 ...
		 <label for="userstatus">Select User Status *</label>
		  <select name="userstatus" id="userstatus">
        <option  value="">Select User Status</option>
         <option  value="active" <?php echo ($row->userstatus == 'active')?"selected='selected'":"" ?> >Active</option>
    <option  value="inactive" <?php echo ($row->userstatus == 'inactive')?"selected='selected'":"" ?>>Inactive</option>
    ...
    <label for="username">
            Username *</label>
        <input id="username" type="text" class="required" name="username" value="<?php echo $row->username; ?>" />
...
<label for="email">
        ...
        <div style="height:33px"><label for="sameadd" style="float:left; ">
            Same as above</label> <input type="checkbox"  id="sameadd" style="width:20px; display:inline"  onClick="copyadd()"/>
        
        </div>
          <label for="coraddress">
            Correspondence Address *</label>
        <textarea id="coraddress" name="coraddress"  cols="40" rows="4"  value="" required><?php echo $row->coraddress; ?></textarea> 
        <label for="contact">
            Contact *</label>
