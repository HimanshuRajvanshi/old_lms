...
//updating leaves
 if(isset($_POST['update']))
		...
		else if(($leavetype == "Optional Holiday")||($leavetype == "Short Leave")||($leavetype == "Comp Off"))
		{
		 $daycount = 0;
		}
		else
		...
		$daycount = mysql_real_escape_string(trim($_POST['daycount']));
		   ...
		   $query = "UPDATE `leave` SET `fromdate` = '$startdate' ,`todate` = '$enddate' , `fromtime`= '$fromtime' , `totime`= '$totime' , `leavetype`= '$leavetype' , `halfdaydetail`= '$halfdaydetail' , `reason`= '$reasons' ,`phone`= '$phone', `daycount`= '$daycount'  WHERE `lid` = $lid";
...
$sql = "Select * from employee where employee.empid = ".$_SESSION[empid]."";		   
$res = mysql_query($sql);
$row = mysql_fetch_object($res);

$getsup = "select * from employee where email = ".$row->supemail."";
$resemp = mysql_query($getsup);
...
$body .= "<p style='font-weight:bold'>Leave Application from ".$row->fname." ".$row->lname."</p><p>Leave Type: ".$leavetype."<br>Start Date: ".$startdate."<br>Half Day Detail: ".$halfdaydetail."<br/> Reasons: ".$reasons."</p>";
...
{
$body = " <div style=\"width: 640px; font-family: Arial, Helvetica, sans-serif; font-size: 14px;background-color:lightblue;padding: 15px;\">";
$body .= "<p style='font-weight:bold'>Leave Application from ".$row->fname." ".$row->lname."</p><p>Leave Type: ".$leavetype."<br>Start Date: ".$startdate."<br>End Date: ".$enddate."<br/> Reasons: ".$reasons."</p>";
$body .= "<br/><a href=\"http://intranet.trilasoft.com\">Login here</a></div>" ;
}
...
{
$body = " <div style=\"width: 640px; font-family: Arial, Helvetica, sans-serif; font-size: 14px;background-color:lightblue;padding: 15px;\">";
$body .= "<p style='font-weight:bold'>Leave Application from ".$row->fname." ".$row->lname."</p><p>Leave Type: ".$leavetype."<br>Start Date: ".$startdate."<br>End Date: ".$enddate."<br>From Time: ".$fromtime."<br>To Time: ".$totime."<br/> Reasons: ".$reasons."</p>";
$body .= "<br/><a href=\"http://intranet.trilasoft.com\">Login here</a></div>" ;
}
...
$mail->AddAddress($row->supemail, $rowemp->fname.' '.$rowemp->lname);
$mail->Subject = 'Leave Application Updated'; 
$mail->MsgHTML($body);
$mail->Send();   
          ...
          <option value="Short Leave" <?php echo ($row->leavetype == 'Short Leave')?"selected='selected'":"" ?>>Short Leave</option>
             ...
             <option value="Planned Leave" <?php echo ($row->leavetype == 'Planned Leave')?"selected='selected'":"" ?>>Planned Leave</option>
              <option value="Optional Holiday" <?php echo ($row->leavetype == 'Optional Holiday')?"selected='selected'":"" ?>>Optional Holiday</option>
               <option value="Emergency Leave" <?php echo ($row->leavetype == 'Emergency Leave')?"selected='selected'":"" ?>>Emergency Leave</option>
			    <option value="Work From Home" <?php echo ($row->leavetype == 'Work From Home')?"selected='selected'":"" ?>>Work From Home</option>
              <option value="Comp Off" <?php echo ($row->leavetype == 'Comp Off')?"selected='selected'":"" ?>>Comp Off</option>
           </select></td>
           <td width="237"><select name="halftype" id="halftype" style="display:none" > 
             <option selected value=""> </option>
             <option value="First Half" <?php echo ($row->halfdaydetail == 'First Half')?"selected='selected'":"" ?> >10:00 am - 2:30 pm</option>
             <option  value="Second Half" <?php echo ($row->halfdaydetail == 'Second Half')?"selected='selected'":"" ?> >2:30 pm - 7:00 pm</option>
             </select>
...
<select name="fromtime" id="fromtime" style="display:none" > 
         ...
         <tr id="totalrow">
           <td>Total Days</td>
           <td colspan="2">
          <label id="totdate" value=""><?php echo $row->daycount; ?></label>
		  <input type="hidden" name="daycount" id="daycount" value="<?php echo $row->daycount; ?>">
           </td>
         </tr>
