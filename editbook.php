...
<div id="content"> 
    ...
    <div class="right_content">  <!-- Start of right content-->    
	 ...
	 $getbook = mysql_query("Select * from books where bookid ='$bookid'");
	 $rowbook = mysql_fetch_object($getbook);
	 
	 if($_POST['update'])
	 {
	$bookname = mysql_real_escape_string(trim($_POST['bookname']));
	$sqlbook = "UPDATE books SET `bookname` = '$bookname' where bookid='$bookid'";
	$resbook = mysql_query($sqlbook);
	if($resbook)
	echo'<h3>The Book has been updated successfully</h3>';
	sleep(3);
	header("location: addbook.php?sessid=".$_SESSION['sessid']."&mess=updatesuccess");
    }
	 ...
	 ?>
		 ...
		 <td><input type="text" name="bookname"  style="width:100%" value="<?php echo $rowbook->bookname; ?>" required />
		 </td></tr>
			<tr>
			<td>&nbsp;</td>
			<td><input type="submit" value="Update" name="update" style="background-color:#cccccc; color:#000000; border:1px solid #000; padding:5px"></td>
