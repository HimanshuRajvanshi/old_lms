...
if($_SESSION['type'] == "2")
{
	...
	$result = mysql_query("SELECT * FROM  `leave` where leave.status = '1' and leave.empid = ".$_SESSION['empid']."", $connection) or die(mysql_error());
   ...
   if($emid)
	...
	if($emid == "myteam")
	{
	$getemail = mysql_query("Select email from employee where employee.empid = '".$_SESSION['empid']."' ", $connection) or die(mysql_error());
	$rowemail = mysql_fetch_object($getemail);	
	$result = mysql_query("Select * from `leave` where status = 1 and leave.supemail = '".$rowemail->email."'", $connection)or die(mysql_error());
    }
