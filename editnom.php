...
$sql = "SELECT * FROM  nominations, employee where nominations.nomid = '$nomid' AND employee.empid = '$employeeid' AND userstatus = 'active' ";
...
$getemp = mysql_fetch_object($res);
 ...
 /*    $mail->IsSMTP(); // telling the class to use SMTP
  ...
  $mail->AddAddress('nagarwal@trilasoft.com', 'Neha Agarwal');
  $mail->Subject = 'Award Nomination'; 
  $mail->MsgHTML($body);
 if( $mail->Send() )
 {
	 $date = date('Y-m-d');
	$sql = "UPDATE nominations SET  awardtype = '$awardtype', quarter = '$quarter', remarks = '$remarks', date =  '$date' where nominations.nomid = '$nomid'";
    $res = mysql_query($sql)or die(mysql_error()); 
	  ...
	  ?>
        ...
        <label for="quarter">
            ...
            <option value="" selected="selected">Select a Quarter</option>
             <option value="Quarter 1 (Jan - Mar)" <?php echo ($row->quarter == 'Quarter 1 (Jan - Mar)')?"selected='selected'":"" ?>>Quarter 1 (Jan - Mar)</option>
             <option value="Quarter 2 (Apr - Jun)" <?php echo ($row->quarter == 'Quarter 2 (Apr - Jun)')?"selected='selected'":"" ?>>Quarter 2 (Apr - Jun)</option>
             <option value="Quarter 3 (Jul - Sep)" <?php echo ($row->quarter == 'Quarter 3 (Jul - Sep)')?"selected='selected'":"" ?>>Quarter 3 (Jul - Sep)</option>
             <option value="Quarter 4 (Oct - Dec)" <?php echo ($row->quarter == 'Quarter 4 (Oct - Dec)')?"selected='selected'":"" ?>>Quarter 4 (Oct - Dec)</option>
            </select>
            ...
            <option value="" selected="selected">Select Award Type</option>
             <option value="Employee of the Quarter" <?php echo ($row->awardtype == 'Employee of the Quarter')?"selected='selected'":"" ?>>Employee of the Quarter</option>
             <option value="Pat on the Back" <?php echo ($row->awardtype == 'Pat on the Back')?"selected='selected'":"" ?>>Pat on the Back</option>
         ...
         <textarea name="remarks" id="remarks" required><?php echo $row->remarks; ?></textarea>   
         <input type="submit" value="Update" name="editnomination" style="width:auto">
