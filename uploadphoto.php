...
$file_exts = array("jpg", "bmp", "jpeg", "gif", "png", "JPG", "JPEG");
...
move_uploaded_file($_FILES["photo"]["tmp_name"], "photos/" . $new_file_name);
echo "<div class='sucess'>"."Stored in: " . "photos/" . $_FILES["photo"]["name"]."</div>";
...
$sql = "UPDATE employee SET photo = '$new_file_name' where empid =  '$empid' ";
...
<div style="padding:5px"><label><h3>Select Passport Sized Photo</h3></label></div>
<div style="padding:5px">
<input type="file" name="photo" />
<div style="font-style:italic; color:#930">(Image size should be less than 1MB in GIF, JPEG or PNG formats)</div>
</div>
<div style="padding:5px">
<input type="submit" name="upload" value="Upload Photo" class="ui-button" />
...
$connection = mysql_pconnect(HOSTNAME,USERNAME,PASSWORD);
mysql_select_db(DATABASE,$connection);
$q1 = "Select photo from employee where empid =  '$empid' ";
$r1 = mysql_query($q1)or die(mysql_error());
$row = mysql_fetch_object($r1);

if($row->photo != '' )
{
echo '<img src="photos/'.$row->photo.'" style="width:48px; height:48px"/>';
}
else
{
echo '<img src="images/noimage.png" style="width:48px; height:48px"/>';
}
